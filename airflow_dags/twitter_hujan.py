...
def run_twitter_extract(**kwargs):
    logging.info("Extract twitter_hujan task is starting")
    ...
    wait_time = 60  # Time to wait between retries (in seconds)
...
def run_twitter_transform(**kwargs):
    ...
    # Check if 'Date Created' is timezone-aware and localize or convert accordingly
    if twitter_data['Date Created'].dt.tz is None:
        ...
        # If already timezone-aware, just convert to Asia/Jakarta
        twitter_data['timestamp_gmt7'] = twitter_data['Date Created'].dt.tz_convert('Asia/Jakarta')

    # Remove timezone information (if any) from the 'timestamp_gmt7'
